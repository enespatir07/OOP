...
Tetromino::Tetromino(const shapes shape) { // Constructor to assign Tetrominos with dynamic 2D char arrays
    if (shape == shapes::I) {
        row = 1, column = 4;
        tetrominoShape = new char *[row];
        for (int i = 0; i < row; i++)
...
Tetromino::~Tetromino() { // Destructor to delete 2D char dynamic char array
    for (int i = 0; i < row; ++i) {
...
void Tetromino::printTetromino() const { // Printing Tetrominos on the screen
...
void Tetromino::rotate(const directions direction) { // Rotates tetrominos either in clockwise or anticlockwise direction
    char **temp = new char *[column];
    for (int i = 0; i < column; i++) // Temporary 2D char array to copy the original char array
        temp[i] = new char[row];

    if (direction == directions::a) { // towards anticlockwise
    ...
    if (direction == directions::c) { // towards clockwise
    ...
    for (int i = 0; i < row; ++i) { // Deleting contents of original array to assign the temporary array to itself
        delete[] tetrominoShape[i];
    }
    delete[] tetrominoShape;
    tetrominoShape = temp;
